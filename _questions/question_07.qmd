...
"What does the PCA plot tell us about the data’s structure?"
...
* No — this would make the clustering predictive rather than exploratory. We use it after clustering to describe the segments.

"All data prep is done automatically."	

* Emphasize the explicit choices in the recipe: what’s included, excluded, and how each transformation works.
...
`step_dummy()` transforms each categorical variable into one or more binary columns (0/1) — one for each category level (except the reference category, unless one_hot = TRUE).
...
These variables were excluded from the preprocessing steps via role assignment — but they still remain in the baked dataset.

We remove them after `bake()` because:

- `employee_number` is a unique ID and doesn't help clustering (and could distort distance).
- `attrition` is an outcome variable, not a predictor — we use it after clustering to describe the segments, not to define them.
...
Look for a point where the curve "bends" — this is often between k = 3 to 4.
